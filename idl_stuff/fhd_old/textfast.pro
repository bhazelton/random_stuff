PRO TextFast,data,header,filename=filename,filepathfull=filepathfull,read=read,write=write,action=action,column_list=column_list,$
 first_line=first_line,xdr=xdr,RootDirectory=RootDirectory,string=string,append=append, use_exact_filename = use_exact_filename
...
IF ptxt EQ -1 and not keyword_set(use_exact_filename) THEN filename_use=filename+String('.txt') ELSE filename_use=filename
...
CASE action OF
    'write':BEGIN
        IF Keyword_Set(column_list) THEN data_use=data[column_list,*] ELSE data_use=data
        ...
        IF Keyword_Set(header) THEN BEGIN
            ...
            IF Keyword_Set(column_list) THEN header_use=header[column_list,*] ELSE header_use=header
    ...
    'read':BEGIN
        ...
        ;  ReadF,unit,data
        ;  Close,unit
        IF Keyword_Set(column_list) THEN data=data[column_list,*]
        RETURN
    END
    ; 'xdr write': BEGIN
