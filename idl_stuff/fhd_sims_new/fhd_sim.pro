pro fhd_sim, data_directory, version = version
  ...
  cube_set = ['even','odd']
  cube_filepath = fhd_path_setup(output_dir=data_directory,output_filename='Sim_obs',subdir='Healpix',version=version)
  ...
  IF Keyword_Set(data_flag) THEN BEGIN
    ...
    IF Keyword_Set(rephase_to_zenith) THEN BEGIN
      ...
      obs.obsx=obs0.zenx
      obs.obsy=obs0.zeny
      obs=vis_struct_init_obs(file_path_vis,hdr,params,n_pol=n_pol,phasera=phasera,phasedec=phasedec,_Extra=extra)
  ...
  for cube_i = 0, 1 do begin
    ...
    FOR obs_i=0,n_files-1 DO BEGIN
      ...
      dirty_arr1=vis_model_freq_split(0,obs,psf,params,flag_arr,model_uv_arr=0,fhd_file_path=file_path_fhd, $
        vis_file_path=file_path_vis, n_avg=n_avg,timing=t_split1,weights=weights_arr1, $
        even_only=even_only,odd_only=odd_only,_Extra=extra)
        
      if cube_i*obs_i eq 0 then begin
        ;; make model uv image
        CASE 1 OF
        ...
        count_ind = 0
        freq_i=0
        pix_ind = (randomu(seed,1)*elements/2)*dimension + randomu(seed,1)*dimension
        while (*weights_arr1[0,freq_i])[pix_ind] lt max(abs(*weights_arr1[0,freq_i]))/10. do begin
          pix_ind = (randomu(seed,1)*elements/2)*dimension + randomu(seed,1)*dimension
        endwhile
        ;;pix_xy = fix([pix_ind mod dimension, pix_ind / dimension])
        ;;patch_size = 11
        ;;pix_list = reform(rebin(reform(indgen(patch_size)+pix_xy[1]-fix(patch_size/2.),1,patch_size),patch_size,patch_size,/sample),patch_size^2)*double(dimension)+ $
        ;;  reform(rebin(indgen(patch_size)+pix_xy[0]-fix(patch_size/2.),patch_size,patch_size,/sample), patch_size^2)
      ...
      model_arr1=vis_model_freq_split(0,obs,psf,params,flag_arr,fhd_file_path=file_path_fhd,vis_file_path=file_path_vis, $
        ...
        variance=variance_arr1, weights=weights_arr1, even_only=even_only,odd_only=odd_only,_Extra=extra)
      ...
      FOR pol_i=0,n_pol-1 DO FOR freq_i=0,n_freq_use-1 DO BEGIN
        ...
        (*dirty_hpx_arr[pol_i,freq_i])[*hpx_ind_map[obs_i]]+=$
          healpix_cnv_apply(dirty_image_generate(*dirty_arr1[pol_i,freq_i],degpix=obs.degpix)*obs.n_vis,hpx_cnv[obs_i])
