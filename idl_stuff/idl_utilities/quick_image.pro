  ...
  if max(abs(imaginary(image))) gt 0 then begin
   ...
   if keyword_set(log) then begin
     ...
     if count_low eq 0 then plot_image = alog10(image) $
        ...
        if count_good eq 0 then message, 'Entire image is 0 or negative -- log scaling will not work.' $
        else print, 'Warning: part of the image is 0 or negative, setting those values to a small value for log scaling'       
