...
if (fits_type eq 3) then begin
    ...
    ;; slightly different approaches) (BH 9/13/2010)
    ...
    pixel_col_num = -1
    ntags = n_elements(tag_names(stc))
    tags = tag_names(stc)
    ...
    select_pix = index_word(tag_names(stc), pixel_col_num,   value=select_name2, err=error2)

    select_map = index_word(tag_names(stc), select_def,value=select_name,  err=error1)
    ;; Added this logic for cases in which the column names were
    ;; changed when they were converted to structure tag names (ie if
    ;; there were white spaces)
