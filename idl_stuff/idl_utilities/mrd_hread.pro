...
;      Added logic to properly calculate data length when the first
;        dimension is zero (as occurs in some radio astronomy files)
;        B. Hazelton February 2012
;-
  On_error,2
  compile_opt idl2
...
; them to be truncated.   Issue a warning but continue if problems are found.
	...
	endwhile
        ...
        if keyword_set(skipdata) then begin 
  			...
  			ndata = product(dims[where(dims gt 0)],/INTEGER) ;; fix radio astronomy fits files with first dim = 0,/integer)
                endif else ndata = 0
